...
供应商关联数据已经写入数据库，前端页面代码也能正常显示关联信息。本次“商品列表仍显示未绑定”的直接原因是后端没有重启，Gateway 仍可能访问到旧的 `dzg-shop` 实例。

处理方式：

1. 停止旧的 `dzg-shop` 实例。
2. 重新启动当前代码的 `dzg-shop`。
3. 在 Nacos 服务列表确认只有目标 Shop 实例在线。
4. 刷新商品管理页面，供应商关联即可显示。

因此，后续遇到“数据库有数据但页面没有变化”的情况，应先确认当前请求是否真正打到了最新后端实例。
...
- 优先显示后端返回的 `supplierNames`。
- 如果后端只返回 `supplierIds`，用当前页面已加载的供应商选项做名称映射。
- 如果两者都没有，显示“未绑定”。

前端不会为了列表显示额外循环调用供应商反查接口，避免商品越多接口越慢。
